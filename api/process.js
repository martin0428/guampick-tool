...
const BRAND_RULES = [
  { keywords: ['bibigo', 'bi bi go'], tag: 'Bibigo' },
  { keywords: ['black & white', 'black and white', 'black&white'], tag: 'Black and White Chef' },
];
...
TRANSLATION RULES:
- Keep brand names as-is (농심→Nongshim, 오뚜기→Ottogi, 삼양→Samyang, 비비고→Bibigo)
...
{"type":"Stationery & Office","title_en":"Sanrio Character Stationery Set","description":"A cute Korean-style desk organizer kit featuring beloved Sanrio characters. Perfect for school or home use.","confidence":0.92}`;
  ...
  try {
    ...
    // 브랜드 태그 감지 (원본 제목 + AI 번역 제목 둘 다 체크)
    const brandTags = detectBrands(title + ' ' + (result.title_en || ''));
    result.brandTags = brandTags;
